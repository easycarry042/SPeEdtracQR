...
<div>
    ...
    <select id="rt-kind" name="kind"
        ...
        <option value="{{ RequestType::KIND_DOCUMENT }}" @selected(old('kind', $requestType?->kind ?? RequestType::KIND_DOCUMENT) === RequestType::KIND_DOCUMENT)>Document / Permit (needs requirements)</option>
        <option value="{{ RequestType::KIND_BOOKING }}" @selected(old('kind', $requestType?->kind) === RequestType::KIND_BOOKING)>Booking / Resource (Phase 2 — scheduling)</option>
    </select>
    <p class="mt-1 text-[12px] text-ink-soft">Booking types don't appear on the public form until Phase 2 (scheduling) ships.</p>
